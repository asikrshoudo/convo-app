...
class _ProfileScreenState extends State<ProfileScreen> {
                  ...
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 24),
                    child: StreamBuilder<DocumentSnapshot>(
                      ...
                      builder: (_, friendSnap) {
                        ...
                        if (isFriend) {
                          return primaryButton('Send Message', () => Navigator.push(context,
                          ...
                          builder: (_, reqSnap) {
                            ...
                            return Row(children: [
                              Expanded(child: OutlinedButton.icon(
                                ...
                                icon: Icon(requestSent ? Icons.hourglass_top_rounded
                                ...
                                onPressed: requestSent ? null : () async {
                                  if (profileMode == 'follow') {
                                    await db.collection('users').doc(myUid).collection('following').doc(widget.uid).set({'uid': widget.uid, 'since': FieldValue.serverTimestamp()});
                                    await db.collection('users').doc(widget.uid).collection('followers').doc(myUid).set({'uid': myUid, 'since': FieldValue.serverTimestamp()});
                                    await db.collection('users').doc(widget.uid).update({'followerCount': FieldValue.increment(1)});
                                    await db.collection('users').doc(myUid).update({'followingCount': FieldValue.increment(1)});
                                  } else {
  ...
  Widget _statBox(String val, String label) => SizedBox(width: 90,
    child: Column(children: [
