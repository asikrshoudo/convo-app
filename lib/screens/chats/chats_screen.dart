...
Future<void> _loadDisappearSetting() async {
  ...
  if (doc.exists && mounted) {
    setState(() =>
        _disappearSeconds = (doc.data() as Map?)?['disappearSeconds']);
  }
}
...
Future<void> _setTyping(bool v) async => db
    .collection('chats')
    .doc(widget.chatId)
    .collection('typing')
    .doc(_myUid)
...
Future<void> _clearUnread() async => db
    .collection('chats')
    .doc(widget.chatId)
    .set({'unread_$_myUid': 0}, SetOptions(merge: true));

void _showEmojiKeyboard() {
  // Show emoji picker (simplified - shows snackbar for now)
...
void _showAttachmentOptions() {
  showModalBottomSheet(
    context: context,
    backgroundColor:
        Theme.of(context).brightness == Brightness.dark ? kCard : Colors.white,
    ...
    builder: (_) => SafeArea(
      child: Wrap(
        children: [
          ListTile(
            leading: const Icon(Icons.image_rounded, color: kGreen),
          ...
          ListTile(
            leading:
                const Icon(Icons.insert_drive_file_rounded, color: kGreen),
    ...
    return Scaffold(
                      ...
                      return StreamBuilder<DocumentSnapshot>(
                        ...
                        builder: (_, snap) {
                          ...
                          return Text(
                            ...
                            style: TextStyle(
                              color: online ? kGreen : Colors.grey,
                              fontSize: 11,
                            ),
                          );
                        },
      ...
      body: Column(
        children: [
          ...
          // Input bar with emoji and attachment buttons
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                // Attachment button
                IconButton(
                  icon: const Icon(Icons.attach_file_rounded, color: kGreen),
