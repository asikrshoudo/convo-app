...
class _FriendsScreenState extends State<FriendsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _sendRequest(String toUid, String toName, String toAvatar) async {
    final fd = await db.collection('users').doc(_myUid).collection('friends').doc(toUid).get();
    if (fd.exists) {
    ...
    if (targetMode == 'follow') {
      await db.collection('users').doc(_myUid).collection('following').doc(toUid).set({'uid': toUid, 'since': FieldValue.serverTimestamp()});
      await db.collection('users').doc(toUid).collection('followers').doc(_myUid).set({'uid': _myUid, 'since': FieldValue.serverTimestamp()});
      await db.collection('users').doc(toUid).update({'followerCount': FieldValue.increment(1)});
      await db.collection('users').doc(_myUid).update({'followingCount': FieldValue.increment(1)});
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Now following $toName'), backgroundColor: kGreen));
    } else {
      final ex = await db.collection('friend_requests')
        .where('from', isEqualTo: _myUid).where('to', isEqualTo: toUid).where('status', isEqualTo: 'pending').get();
      if (ex.docs.isNotEmpty) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Request already sent!')));
        return;
      }
  ...
  Future<void> _accept(String docId, String fromUid) async {
    ...
    await db.collection('users').doc(_myUid).collection('friends').doc(fromUid).set({'uid': fromUid, 'since': FieldValue.serverTimestamp()});
    await db.collection('users').doc(fromUid).collection('friends').doc(_myUid).set({'uid': _myUid, 'since': FieldValue.serverTimestamp()});
    await db.collection('users').doc(_myUid).update({'friendCount': FieldValue.increment(1)});
    await db.collection('users').doc(fromUid).update({'friendCount': FieldValue.increment(1)});
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          controller: _tab,
          indicatorColor: kGreen, labelColor: kGreen, unselectedLabelColor: Colors.grey,
          tabs: const [
      ...
      body: TabBarView(controller: _tab, children: [
        ...
        Column(children: [
          ...
          if (_searching)
            const Padding(padding: EdgeInsets.all(16), child: CircularProgressIndicator(color: kGreen))
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                ...
                trailing: widget.startChat
                  ? FilledButton(
                      style: FilledButton.styleFrom(backgroundColor: kGreen, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                      onPressed: () {
                        final ids    = [_myUid, u['uid'] as String]..sort();
                        final chatId = ids.join('_');
                        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ChatScreen(
                          otherUid: u['uid'], otherName: u['name'] ?? 'User',
                          otherAvatar: u['avatar'] ?? '?', chatId: chatId)));
                      },
                      child: const Text('Message'))
                  : StreamBuilder<DocumentSnapshot>(
                      stream: db.collection('users').doc(_myUid).collection('friends').doc(u['uid'] as String).snapshots(),
                      builder: (_, fSnap) {
                        ...
                        return OutlinedButton(
                          style: OutlinedButton.styleFrom(side: const BorderSide(color: kGreen), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                          onPressed: () => _sendRequest(u['uid'], u['name'] ?? 'User', u['avatar'] ?? 'U'),
                          child: Text(u['profileMode'] == 'follow' ? 'Follow' : 'Add',
            ...
            return ListView(children: docs.map((doc) {
              ...
              return ListTile(
                ...
                trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  GestureDetector(
                    onTap: () => db.collection('friend_requests').doc(doc.id).update({'status': 'declined'}),
                    child: Container(width: 38, height: 38,
        ...
        // ── My Friends tab ───────────────────────────────────────────────
        StreamBuilder<QuerySnapshot>(
          stream: db.collection('users').doc(_myUid).collection('friends').snapshots(),
          builder: (_, snap) {
            if (!snap.hasData || snap.data!.docs.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  return ListTile(
                    ...
                    trailing: FilledButton.icon(
                      ...
                      onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(
                        otherUid: doc.id, otherName: u['name'] ?? 'User',
                        otherAvatar: u['avatar'] ?? '?', chatId: chatId)))),
